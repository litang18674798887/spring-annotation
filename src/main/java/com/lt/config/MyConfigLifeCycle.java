 ...
 * 销毁
 *      单实例：容器关闭的时候
 *      多实例：容器不会管理这个bean,容器不会调用销毁的方法
 *
 *
 *
 *
 *   遍历得到容器中所有的BeanPostProcessor,挨个执行postProcessBeforeInitialization，
 *      一旦返回null,跳出for循环，不会执行后面的BeanPostProcessor.applyBeanPostProcessorsBeforeInitialization
 *
 *
 *
 *   BeanPostProcessor原理
 ...
 *  3.可以JSR250:
 *      @PostConstruct : 在bean创建完成并且属性值赋值完成，来执行初始化
 ...
 *      bean赋值，注入其他组件，@autowired,生命之前注解功能，@Async,xxx BeanPostProcessor
 */

@ComponentScan("com.lt.bean")
@Configuration
